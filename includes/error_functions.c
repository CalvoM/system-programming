...
static void terminate(Boolean useExit3){
    char *s;
    //Check if dumpcore envvar is defined and is not an empty string; or check `useExit3` variable
 ...
 *          2. calls _exit() instead of exit() - terminate without flush stdio or invoking exit handlers
 *          Useful for library fxn that creates child process and needs to terminate because of an error
*/
...
/**
 * @brief same as errExit() but prints the error text corresponnding to value of errno
 * Used with POSIX threads API
*/
...
/**
 * @brief  Diagnose general errors including errors from library functions that don't set errno
 * Appends '\n' to 
*/
