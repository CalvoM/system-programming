...
int main(int argc, char** argv){
    ...
    if(argc < 3 || strcmp(argv[1],"--help")==0) usageErr("%s file {r<length>|R<length>|w<string>|s<offset>}...\n",argv[0]);
    int inFlags = O_RDWR | O_CREAT; //Read-Write and Create
    mode_t inPerms = S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH;//rw-rw-rw
    fd = open(argv[1],inFlags,inPerms);
    if(fd==-1) errExit("Error opening: %s",argv[1]);
    for (ap=2;ap<argc;ap++){
        ...
        switch(argv[ap][0]){
            ...
            case 'R':
                ...
                buf = malloc(len);
                if (buf == NULL) errExit("malloc error");
            ...
            case 'w':
                ...
                if(numWritten == 1) errExit("Write Error");
