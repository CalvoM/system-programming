...
int main(int argc, char** argv){
    ...
    mode_t outputPerms = S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH;
    int opt;
    ...
    if(close(output_fd)==-1) errExit("Error closing file");
    if(close(input_fd)==-1) errExit("Error closing file");
    input_fd = open(argv[i],O_RDONLY);
    i++;
    }
     ssize_t read_bytes;
    while((read_bytes = read(input_fd,read_buf,read_buf_size))!=0){
        if(write(STDOUT_FILENO,read_buf,read_bytes)!= read_bytes) fatal("Error while writing to file");
    }
    if(close(input_fd)==-1) errExit("Error closing file");
    exit(EXIT_SUCCESS);
}